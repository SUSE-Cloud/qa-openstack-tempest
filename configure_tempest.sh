...
echo "Querying for the EC2 information of user..."
EC2User="admin"
aws_access=$(keystone ec2-credentials-list | grep "|\s*${EC2User}\s*|" | awk -F\| '{print $3}')
aws_secret=$(keystone ec2-credentials-list | grep "|\s*${EC2User}\s*|" | awk -F\| '{print $4}')

if [ ! -z "$aws_access" -a ! -z "$aws_secret" ]; then
  sed -i -e "s/aws_access = .*/aws_access = $aws_access/" \
         -e "s/aws_secret = .*/aws_secret = $aws_secret/" \
       $CONF_PATH
else
